 ...
 */
function theme_opendev_extra_options( $option ){
 // get this theme key
...
add_filter( 'fa_extra_slider_options', 'theme_opendev_extra_options' );

/**
 * Output for the fields above
 * @param array $options - current options set
 */
function opendev_layout_fields_output( $post ){
 // get the stored values of the options implemented by this theme
 $theme_options = fa_get_theme_options( __FILE__, $post );
 $theme_details = fa_theme_opendev_details( false );
?>
<h2><?php printf( __('%s : theme specific settings', 'fapro'), $theme_details['name']);?></h2>
<table class="form-table">
 <tbody>
  <tr>
   <th><label for="opendev_timer"><?php _e('Show timer', 'fapro');?>:</label></th>
   <td>
    <input id="opendev_timer" type="checkbox" name="<?php fa_theme_var_name('show_timer', __FILE__);?>" value="1" <?php fa_checked( $theme_options['show_timer'] )?> />
...
add_action('fa_theme_layout_settings-' . fa_get_theme_key( __FILE__ ), 'opendev_layout_fields_output', 10, 1);
 ...
 */
function fa_theme_opendev_details( $defaults ){
 $defaults = array(
  'author'		=> 'Alex Corbi',
  'authorURI'		=> 'http://www.lifeformapps.com',
  'copyright'		=> 'author',
  'compatibility'	=> '3.0',
  'version'		=> '1.0',
  'name'			=> 'opendev',
...
add_filter('fa-theme-details-' . fa_get_theme_key( __FILE__ ), 'fa_theme_opendev_details', 1);

/**
 * CSS customizations
 */
function fa_theme_css_opendev(){
 $rules = array(
  'container' => array(
   'css_selector' => '.fa_slider_opendev', // all child elements from container will descend from this
